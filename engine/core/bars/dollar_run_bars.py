...
def build_dollar_run_bars(
    ...
    *,
    ...
    for i, row in enumerate(df.iter_rows(named=True)):
        ...
        dollar = row["price"] * row["qty"] * sign
        bar_ticks.append(row)

        if prev_sign is None or sign == prev_sign:
            run_dollar += abs(dollar)
        else:
            run_history.append(run_dollar)
            run_dollar = abs(dollar)
        prev_sign = sign
        ...
        ema = (
            sum(run_history[-ema_span:]) / len(run_history[-ema_span:])
            if run_history
            else 1.0
        )
        threshold = alpha * ema

        if run_dollar >= threshold:
