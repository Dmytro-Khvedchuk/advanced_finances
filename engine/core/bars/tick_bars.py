...
def build_tick_bars(data, bar_size: int = 10, drop_last_incomplete: bool = False) -> pl.DataFrame:
    ...
    bars = (
        ...
        .agg([
            ...
            pl.when(pl.col("buyer_taker")).then(pl.col("qty")).otherwise(0.0).sum().alias("buy_volume"),
            (~pl.col("buyer_taker")).cast(pl.Int64).sum().alias("sell_ticks"),
            pl.when(~pl.col("buyer_taker")).then(pl.col("qty")).otherwise(0.0).sum().alias("sell_volume"),

            pl.when(pl.col("buyer_taker")).then(1).otherwise(-1).sum().alias("signed_tick_sum"),
            pl.when(pl.col("buyer_taker")).then(pl.col("qty")).otherwise(-pl.col("qty")).sum().alias(
                "signed_volume_sum"),
