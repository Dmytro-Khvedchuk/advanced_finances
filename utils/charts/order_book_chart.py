...
def _can_fill_notional_at_best(levels: list[Tuple[float, float]], dollars: float) -> bool:
...
def _can_fill_notional_across_book(levels: list[Tuple[float, float]], dollars: float) -> bool:
...
def make_orderbook_bar_from_lists(
    ...
    *,
    ...
    p.hbar(y="price", right="qty", height=0.8, source=bid_src,
           fill_alpha=0.7, fill_color="#2ca02c",
           line_color="#0b3d0b", line_width=2, legend_label="Bids")
    p.hbar(y="price", right="qty", height=0.8, source=ask_src,
           fill_alpha=0.7, fill_color="#d62728",
           line_color="#5c0000", line_width=2, legend_label="Asks")
    ...
    if bid_pairs:
        p.add_layout(Span(location=best_bid, dimension="width",
                          line_color="#0b3d0b", line_width=3, line_alpha=1.0))
    if ask_pairs:
        p.add_layout(Span(location=best_ask, dimension="width",
                          line_color="#5c0000", line_width=3, line_alpha=1.0))
    ...
    if all_prices:
        ...
        if bid_pairs:
            p.add_layout(Label(x=x_left, y=best_bid,
                               text=f"Best Bid {best_bid:,.2f}",
                               text_color="#0b3d0b", text_font_style="bold",
                               text_alpha=0.95, x_offset=0, y_offset=0))
        if ask_pairs:
            p.add_layout(Label(x=x_right, y=best_ask,
                               text=f"Best Ask {best_ask:,.2f}",
                               text_color="#5c0000", text_font_style="bold",
                               text_alpha=0.95, x_offset=0, y_offset=0))
    ...
    ) if (bid_pairs and ask_pairs) else "<i>No data</i>", width=width)
